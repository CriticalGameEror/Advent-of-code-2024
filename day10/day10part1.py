...
for start_cord in coords_dict[0]:
    ...
    while len(lookup_queue) != 0:
        ...
        # below is for efficiency as it ensures no backtracking occurs
        if (current_lookup[0], current_lookup[1]) in visited_locations:
            continue
        elif current_lookup[2] == 9:
            total += 1
        else:
            lookup_queue += find_sorrounding(
                current_lookup[2]+1, (current_lookup[0], current_lookup[1]), coords_dict)
        visited_locations.add((current_lookup[0], current_lookup[1]))
print(total)
