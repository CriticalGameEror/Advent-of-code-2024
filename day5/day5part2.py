    ...
    for index, current_number in enumerate(number_list):
        ...
        for future_number in number_list[index+1:]:
            if future_number not in come_before[current_number]:
                ...
                break
    if list_broken:
        while list_broken is True:
            list_broken = False
            # this orders the list correctly but the other way around
            for x in range(1, len(number_list)):
                if number_list[x-1] not in come_before:
                    continue
                if number_list[x] in come_before[number_list[x-1]]:
                    number_list[x-1], number_list[x] = number_list[x], number_list[x-1]
                    list_broken = True
